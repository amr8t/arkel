pub fn verify_request(
    ...
    if now.abs_diff(t) > 60 {
        return Err("x-arkel-time outside skew window".to_string());
    }
    let auth = headers
        .get("authorization")
        .and_then(|v| v.to_str().ok())
    ...
    let pubkey = iroh::PublicKey::from_bytes(
        pk_bytes
            .as_slice()
            .try_into()
            .map_err(|_| "bad pubkey len")?,
    )
    .map_err(|_| "bad pubkey")?;
    let sig =
        iroh::Signature::from_bytes(sig_bytes.as_slice().try_into().map_err(|_| "bad sig len")?);
    let payload = format!(
        "{method} {path} {} {t}",
